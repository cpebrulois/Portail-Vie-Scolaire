 ...
 * ============================================================
 * Patch de agora_pixhare.js : remplace l'API Mistral par window.LocalLLM.
 * Pre-requis : <script src="local_llm.js"></script> AVANT ce fichier.
 *
 * Adaptations Qwen 0.5B (testees GPT5.5+Aurelien) :
 *  - SOCLE_COMMUN (interdits anti-recopie) prefixe le system prompt
 *  - Sampling : temp 0.05 (au lieu de 0.95) - Qwen 0.5B est tres litteral
 *  - maxTokens : 160 conversation, 200 analyse (au lieu de 1000)
 *  - Pour signalement Supabase : signaler clairement que le LLM ne doit
 *    PAS inventer de JSON. Le code JS valide les champs avant envoi.
 *
 * RGPD : aucune cle API, aucun appel reseau a un service tiers.
 * ============================================================ */
(function(){
  // KERN_KEY/URL/MODEL supprimes - on utilise window.LocalLLM
  var KERN_TEMP=0.05;             // reduit de 0.95 (Qwen 0.5B litteral)
  var KERN_ANALYSIS_TEMP=0.0;     // signalement = deterministe
  var KERN_MAX_TOKENS=160;        // reduit de 1000 (Qwen 0.5B se perd)
  ...
  function pushMessage(role,text){HISTORY.push({role:role,content:text,ts:new Date().toISOString()});saveHistory();renderMessages();updateControls();if(role==='assistant'&&typeof window.PVS_VITS_SPEAK==='function')window.PVS_VITS_SPEAK(text);}
  ...
  async function complete(messages,temp,maxTokens){
    if(!window.LocalLLM){
      throw new Error("LocalLLM absent. Ajoute <script src=\"local_llm.js\"></script> avant agora_pixhare.local.js.");
    }
    if(!window.LocalLLM.isReady()){
      throw new Error("Assistant local non active. Clique sur 'Activer l'assistant local'.");
    }
    // Prefixe le system prompt original par le SOCLE_COMMUN (interdits anti-recopie).
  ...
  window.prepareKernConcern=async function(){
    /* AVERTISSEMENT : Qwen 0.5B est trop petit pour produire un JSON
       parfaitement structure de maniere fiable. Le code JS ci-dessous
       valide les champs apres generation. Si un champ est absent ou
       suspect, on tombe en mode formulaire semi-guide (a coder en V2.1).
       Pour l'instant : on garde la logique originale, en sachant que
       certaines extractions JSON peuvent rater - une revalidation humaine
       (CPE, AED) reste indispensable avant envoi Supabase. */
